...
/// <summary>
/// v0.9.12 source-path single-enemy replay adapter.
///
/// v0.9.12 starts reducing the preferred[] trace dependency in a guarded way:
/// deterministic 0x2E97 rotate tuples are generated by the model and used by
/// the movement step, while random/R-register and BFS/chase override tuples still
/// fall back to the trace.
///
/// Scope:
/// - reference trace remains the source for player, gates, timers, release timing,
///   random/BFS preferred[] cases and VRAM/tile state;
...
/// movement window, without pretending that release timing, random preferred[],
/// BFS/chase preferred overrides, gates, chase, or multi-enemy scheduling are
/// autonomous yet.
/// </summary>
public sealed class LadyBugSourcePathSingleEnemyReplayAdapter : IEnemySimulationAdapter
{
    private const string Version = "v0.9.12";

    public string Name => "Lady Bug source-path single-enemy replay";

    public string Description =>
        "v0.9.12: compute the active mono-enemy transition with the source-first path; " +
        "use modeled 0x2E97 rotate preferred[] when safe, and sync random/BFS preferred[], release timing, player, gates, VRAM context and multi-enemy frames from the reference trace.";
    ...
    public SimulationAdapterResult Run(IReadOnlyList<EnemyTraceFrame> referenceFrames)
    {
        ...
        for (int i = 1; i < referenceFrames.Count; i++)
        {
            ...
            if (!LadyBugPreferredHybridProvider.TryGetPreferred(resultFrame, startEnemy.Slot, out LadyBugPreferredHybridProvider.Result preferredResult))
            {
    ...
    private static string BuildSummary(ReplayStats stats)
    {
        ...
        builder.Append("releaseMode=reference-synced, preferredMode=hybrid-modeled-2E97-rotate-else-trace-synced, environmentMode=trace-synced, ");
        ...
        if (!string.IsNullOrWhiteSpace(stats.FirstPreferredModeledRotate))
        {
            builder.Append("; ");
            builder.Append(stats.FirstPreferredModeledRotate);
        }

        if (!string.IsNullOrWhiteSpace(stats.FirstPreferredTraceFallback))
        {
            builder.Append("; ");
            builder.Append(stats.FirstPreferredTraceFallback);
        }

        builder.Append("; NOTE: v0.9.12 default candidate computes the active single-enemy movement step through the source path. It uses modeled 0x2E97 rotate preferred[] tuples when the trace-independent rotate model matches the tuple, and still syncs release timing, random/BFS preferred[] cases, player, gates, timers, VRAM context, inactive slots, and multi-enemy frames from MAME.");
