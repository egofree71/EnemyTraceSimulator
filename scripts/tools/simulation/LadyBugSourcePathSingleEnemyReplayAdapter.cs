...
///   preferred[] and VRAM/tile state;
/// - when exactly one enemy is active before and after a transition, the adapter
///   computes that enemy's next raw/x/y state with the source path:
///     0x427E -> 0x42E6 / 0x4325 / 0x4241 -> 0x43BA;
/// - when no enemy is active, when the first enemy is released, or when multiple
///   enemies are active, the adapter syncs back to the reference and records the
///   reason in the summary.
///
/// This makes the left board a real candidate for the validated single-enemy
/// movement window, without pretending that release timing, preferred[] generation,
/// gates, chase, or multi-enemy scheduling are autonomous yet.
/// </summary>
public sealed class LadyBugSourcePathSingleEnemyReplayAdapter : IEnemySimulationAdapter
{
    ...
    public SimulationAdapterResult Run(IReadOnlyList<EnemyTraceFrame> referenceFrames)
    {
        ...
        for (int i = 1; i < referenceFrames.Count; i++)
        {
            ...
            if (activeStart != 1 || activeResult != 1)
            {
                ...
                if (string.IsNullOrEmpty(stats.FirstMultiEnemySync))
                {
                    stats.FirstMultiEnemySync =
                        $"firstMultiEnemySync startIndex={i - 1} startTick={referenceFrames[i - 1].frame} " +
            ...
            if (startEnemy.Slot != referenceResultEnemy.slot)
            {
                ...
                if (string.IsNullOrEmpty(stats.FirstSlotChangeSync))
                {
                    stats.FirstSlotChangeSync =
                        $"firstSlotChangeSync startIndex={i - 1} startTick={referenceFrames[i - 1].frame} " +
            ...
            if (!TryGetPreferred(resultFrame, startEnemy.Slot, out int preferred))
            {
            ...
            SimulationEnemyWorkState modeledWork = BuildModeledEnemyWork(resultFrame.enemyWork, step, preferred);
        ...
        ref int testedDirections)
    {
        ...
        for (int attempt = 1; attempt <= 8; attempt++)
        {
            ...
            if (!alreadyRejected)
            {
                ...
                // Source-faithful detail: 0x4241 uses A/DAT_61C8 and B as a local
                // fallback scan mask. It does not write failed fallback candidates
                // back into 0x61C1. Only the preferred and current direction
                // rejections around 0x4315 / 0x4331 are committed to 0x61C1.
            }
        ...
        int preferred)
    {
        ...
        if (work.Preferred.Count == 0)
        {
            for (int i = 0; i < LadyBugMonsterPreferenceSystem.PreferredSlotCount; i++)
                work.Preferred.Add(0);
        }

        if (step.Slot >= 0 && step.Slot < work.Preferred.Count)
            work.Preferred[step.Slot] = preferred & 0x0F;
    ...
    private static bool TryGetPreferred(EnemyTraceFrame frame, int slot, out int preferred)
    {
        ...
        return IsOneHotDirection(preferred);
    }
    ...
    private static string BuildSummary(ReplayStats stats)
    {
        ...
        builder.Append("releaseMode=reference-synced, preferredMode=trace-synced, environmentMode=trace-synced, ");
        builder.Append("multiEnemyMode=reference-sync-after-scope-limit");
        ...
        if (!string.IsNullOrWhiteSpace(stats.FirstMultiEnemySync))
        {
            builder.Append("; ");
