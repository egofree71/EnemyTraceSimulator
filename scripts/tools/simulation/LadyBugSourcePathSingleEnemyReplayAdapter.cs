...
/// <summary>
/// v0.9.15 source-path single-enemy replay adapter.
///
...
/// - 0x2EC7 random base tuples can remain trace-synced or be generated by a seeded C# Random for behavior testing.
///
...
{
    private const string Version = "v0.9.15";

    public string Name => "Lady Bug source-path single-enemy replay";

    public string Description =>
        "v0.9.15: compute active mono-enemy movement with the source path; " +
        "use modeled 0x2E97 rotate and visible 0x477D BFS/chase preferred[] overrides; " +
        "optionally replace trace-synced 0x2EC7 random tuples with seeded C# Random for behavior testing.";

    public bool ExpectedToMismatch => LadyBugEnemySimulationSettings.LoadOrDefault().ExpectedToDivergeFromTrace;
    ...
    public SimulationAdapterResult Run(IReadOnlyList<EnemyTraceFrame> referenceFrames)
    {
        ...
        LadyBugEnemySimulationSettings settings = LadyBugEnemySimulationSettings.LoadOrDefault();
        var preferredContext = new LadyBugPreferredHybridProvider.Context(settings);
        var stats = new ReplayStats { Frames = referenceFrames.Count, Settings = settings };
    ...
    private static string BuildSummary(ReplayStats stats)
    {
        ...
        builder.Append($"preferredModeledCSharpRandomTransitions={stats.PreferredModeledCSharpRandomTransitions}, ");
        builder.Append($"preferredTraceFallbackTransitions={stats.PreferredTraceFallbackTransitions}, ");
        builder.Append($"randomInputMode={stats.Settings.RandomModeLabel}, ");
        builder.Append($"csharpRandomSeed={stats.Settings.CSharpRandomSeed}, ");
        builder.Append($"randomDivergenceExpected={(stats.Settings.ExpectedToDivergeFromTrace ? "true" : "false")}, ");
        ...
        builder.Append(stats.Settings.UsesCSharpRandom
        ...
        AppendOptional(builder, stats.FirstPreferredModeledCSharpRandom);
        AppendOptional(builder, stats.FirstPreferredTraceFallback);

        builder.Append(stats.Settings.UsesCSharpRandom
            ? "; NOTE: v0.9.15 computes the active single-enemy movement step through the source path. It uses modeled 0x2E97 rotate, modeled visible 0x477D BFS/chase overrides, and seeded C# random for 0x2EC7-style random base tuples. Divergence from the MAME reference is expected in this mode; use it to inspect left-board behavior, not bit-perfect replay."
    ...
    private sealed class ReplayStats
    {
        ...
        public int PreferredModeledCSharpRandomTransitions;
        ...
        public string FirstPreferredModeledCSharpRandom = string.Empty;
        public string FirstPreferredTraceFallback = string.Empty;
        public LadyBugEnemySimulationSettings Settings = new();
        ...
        public void RecordPreferredProvider(LadyBugPreferredHybridProvider.Result result)
        {
            ...
            else
            {
                ...
                if (result.UsedModeledCSharpRandom)
                {
                    PreferredModeledCSharpRandomTransitions++;
                    if (string.IsNullOrEmpty(FirstPreferredModeledCSharpRandom))
                        FirstPreferredModeledCSharpRandom = "firstPreferredModeledCSharpRandom source=" + result.Source;
                }

                if (result.UsedTraceFallback || (!result.UsedModeledBfs && !result.UsedModeledCSharpRandom))
                {
